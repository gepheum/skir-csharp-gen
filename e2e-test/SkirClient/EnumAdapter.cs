...
/// </para>
/// </summary>
public sealed class EnumAdapter<T> : Serializer<T> where T : class
{
    ...
    internal override bool IsDefaultInternal(T value) => _getKindOrdinal(value) == 0;
    ...
    private T DecodeImpl(byte[] data, ref int offset, bool keep)
    {
        ...
        if (ko < _kindOrdinalToEntry.Count && _kindOrdinalToEntry[ko] is IVariantEntry entry)
        {
            if (entry.Constant is T c)
            {
                Serializers.SkipValue_(data, ref offset);
                return c;
            }
            return entry.DecodeWrap(data, ref offset, keep);
        }
        Serializers.SkipValue_(data, ref offset);
        return _default;
    }
